...
from .forms import CustomSignupForm, AgentSignupForm, PropertyCreateForm, TenantCreateForm, LeaseCreateForm, MaintenanceRequestForm, InspectionScheduleForm
...
# ==================== AGENT CREATION VIEWS ====================

@login_required
def agent_property_create_view(request):
    agent = get_agent_or_redirect(request)
    if not agent:
        return redirect('frontend:home')
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            property_obj.save()
            messages.success(request, f"Property '{property_obj.property_name}' created successfully!")
            return redirect('frontend:agent_properties')
    else:
    ...
    return render(request, 'frontend/agent_property_form.html', context)


@login_required
def agent_tenant_create_view(request):
    agent = get_agent_or_redirect(request)
    if not agent:
        return redirect('frontend:home')
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            tenant.agent = agent
            tenant.save()
            messages.success(request, f"Tenant '{tenant.property_name}' added successfully!")
            return redirect('frontend:agent_tenants')
    else:
    ...
    return render(request, 'frontend/agent_tenant_form.html', context)


@login_required
def agent_lease_create_view(request):
    agent = get_agent_or_redirect(request)
    if not agent:
        return redirect('frontend:home')

    if request.method == 'POST':
        form = LeaseCreateForm(request.POST, agent=agent)
        if form.is_valid():
            lease = form.save(commit=False)
            lease.agent = agent
            lease.save()
            messages.success(request, "Lease created successfully!")
            return redirect('frontend:agent_leases')
    else:
        form = LeaseCreateForm(agent=agent)
    ...
    return render(request, 'frontend/agent_lease_form.html', context)


@login_required
def agent_maintenance_create_view(request):
    agent = get_agent_or_redirect(request)
    if not agent:
        return redirect('frontend:home')

    if request.method == 'POST':
        form = MaintenanceRequestForm(request.POST, agent=agent)
        if form.is_valid():
            maintenance = form.save()
            messages.success(request, "Maintenance request submitted successfully!")
            return redirect('frontend:agent_maintenance')
    else:
        form = MaintenanceRequestForm(agent=agent)
    ...
    return render(request, 'frontend/agent_maintenance_form.html', context)


@login_required
def agent_inspection_create_view(request):
    agent = get_agent_or_redirect(request)
    if not agent:
        return redirect('frontend:home')

    if request.method == 'POST':
        form = InspectionScheduleForm(request.POST, agent=agent)
        if form.is_valid():
            inspection = form.save()
            messages.success(request, "Inspection scheduled successfully!")
            return redirect('frontend:agent_inspections')
    else:
        form = InspectionScheduleForm(agent=agent)
    ...
    return render(request, 'frontend/agent_inspection_form.html', context)
